...
{
    // Validate the input, only Callsub is required
    $request->validate([
    ...
    $url = 'http://10.55.1.143:8983/api/CRMApi/GetSimDetails';
    $response = Http::withHeaders([
        ...
        'UserId' => $userId,  // Use the default UserId
    ]);

    // Check if the response is successful
    if ($response->successful()) {
        return redirect()->route('sim-form')->with('response', json_encode($response->json(), JSON_PRETTY_PRINT));
    } else {
        return redirect()->route('sim-form')->with('response', 'Failed to fetch data');
    }
}
...
private function callPingBukAPI($phoneNumber)
{
    ...
    public function handleIncomingMessage(Request $request)
    {
        ...
        $chatwootUrl = "https://app.chatwoot.com/api/v1/accounts/105954/conversations"; // Use your account ID
        $inboxId = "49841"; // Chatwoot inbox ID
        $data = [
            'content' => $userMessage,
            'inbox_id' => $inboxId,
            'contact' => [
        ...
        $authToken = 'fc6f6e296de2ef3e317054d2ddb6f223';
        $twilioNumber = 'whatsapp:+14155238886';

        $client = new Client($accountSid, $authToken);
        try {
            // Send request to Chatwoot
            $response = $client->post($chatwootUrl, [
                'json' => $data,
                'headers' => [
                    'Content-Type' => 'application/json',
                    'Authorization' => 'Bearer your_chatwoot_access_token', // Only if required
                ],
            ]);
    
            // Log success response
            Log::info('Message sent to Chatwoot: ' . $response->getBody());
        } catch (\Exception $e) {
            // Log any error
            Log::error('Error connecting to Chatwoot: ' . $e->getMessage());
        }
        ...
        if (in_array(strtolower($userMessage), ['hi', 'hello', 'morning', 'good morning', 'asc'])) {
            ...
            switch ($userMessage) {
                ...
                case '7': // Sim-card
                    ...
                    break;
                case '8': // Sim-card
                    $responseMessage = "You are being connected to a live agent. Please hold on.";
                    // Call function to connect with Chatwoot
                    $this->connectWithChatwoot($from);
    
                    $this->sessionData[$from]['menu_state'] = 'agent';
                    break;
                default:
    ...
    // chatwood function when bit connect for agent
    private function connectWithChatwoot($from)
{
    // Chatwoot API endpoint
    $chatwootUrl = "https://app.chatwoot.com/api/v1/accounts/105954/conversations";

    $inboxId = "49849"; // Use the correct Chatwoot inbox ID
    $apiKey = 'XwED1eRvR1qW6v5DobzViET5';
    ...
    $postData = [
        ...
        "message" => [
            "content" => "User has requested to connect with an agent.",
        ]
    ];

    // Use GuzzleHTTP to send the request to Chatwoot
    try {
        ...
        if ($response->getStatusCode() == 200) {
            \Log::info('Conversation created successfully in Chatwoot');
        } else {
            \Log::error('Failed to create conversation in Chatwoot. Status: ' . $response->getStatusCode());
        }
    } catch (\Exception $e) {
        \Log::error('Error connecting to Chatwoot: ' . $e->getMessage());
        \Log::error('Request Headers: ' . json_encode($headers)); // Log headers for debugging
        \Log::error('Request Body: ' . json_encode($postData)); // Log request body
        \Log::error('Response Body: ' . $e->getResponse()->getBody()); // Log response body
    }
}

    
    // // chatwood function when bit connect for agent
    // private function connectWithChatwootForAgent($from)
    // {
    //     // Chatwoot API endpoint
    //     $chatwootUrl = "https://app.chatwoot.com/api/v1/accounts/
    //     $inboxId = "49849"; // Use the correct Chatwoot inbox ID
    //     $apiKey = 'XwED1eRvR1qW6v5D
    //     // Data for the new conversation
    //     $postData = [
